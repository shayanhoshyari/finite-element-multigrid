#ifndef BRDISON_VEC_H
#define BRDISON_VEC_H
...
#define TYPEDEF_VEC(dim)                   \
  typedef Vec<dim, double> Vec ## dim ## d;          \
  typedef Vec<dim, float> Vec ## dim ## f;           \
  typedef Vec<dim, int> Vec ## dim ## i;             \
  typedef Vec<dim, unsigned int> Vec ## dim ## ui;   \
  typedef Vec<dim, short> Vec ## dim ## s;           \
  typedef Vec<dim, unsigned short> Vec ## dim ## us; \
  typedef Vec<dim, char> Vec ## dim ## c;            \
  typedef Vec<dim, unsigned char> Vec ## dim ## uc
...
template<unsigned int N, class T>
inline Vec<N, T>
min_union(const Vec<N, T> & a, const Vec<N, T> & b)
{
  Vec<N, T> m;
...
template<unsigned int N, class T>
inline Vec<N, T>
max_union(const Vec<N, T> & a, const Vec<N, T> & b)
{
  Vec<N, T> m;
  for(unsigned int i = 0; i < N; ++i) (a.v[i] > b.v[i]) ? m.v[i] = a.v[i] : m.v[i] = b.v[i];
  return m;
...
template<class T>
inline Vec<2, T>
rotate(const Vec<2, T> & a, float angle)
{
  T c = cos(angle);
  T s = sin(angle);
...
{
  return Vec<3, T>(
    a.v[1] * b.v[2] - a.v[2] * b.v[1], a.v[2] * b.v[0] - a.v[0] * b.v[2], a.v[0] * b.v[1] - a.v[1] * b.v[0]);
}
...
template<unsigned int N, class T>
inline void
assign(const Vec<N, T> & a, T & a0, T & a1)
{
  assert(N == 2);
  a0 = a.v[0];
  a1 = a.v[1];
...
template<unsigned int N, class T>
inline void
assign(const Vec<N, T> & a, T & a0, T & a1, T & a2)
{
  assert(N == 3);
  a0 = a.v[0];
  a1 = a.v[1];
...
template<unsigned int N, class T>
inline void
assign(const Vec<N, T> & a, T & a0, T & a1, T & a2, T & a3)
{
  assert(N == 4);
  a0 = a.v[0];
  a1 = a.v[1];
...
template<unsigned int N, class T>
inline void
assign(const Vec<N, T> & a, T & a0, T & a1, T & a2, T & a3, T & a4, T & a5)
{
  assert(N == 6);
  a0 = a.v[0];
  a1 = a.v[1];
...
template<unsigned int N, class T>
inline void
minmax(const Vec<N, T> & x0, const Vec<N, T> & x1, Vec<N, T> & xmin, Vec<N, T> & xmax)
{
...
minmax(const Vec<N, T> & x0,
       ...
       Vec<N, T> & xmax)
{
  for(unsigned int i = 0; i < N; ++i) minmax(x0.v[i], x1.v[i], x2.v[i], x3.v[i], xmin.v[i], xmax.v[i]);
}
...
minmax(const Vec<N, T> & x0,
       ...
       Vec<N, T> & xmax)
{
  for(unsigned int i = 0; i < N; ++i) minmax(x0.v[i], x1.v[i], x2.v[i], x3.v[i], x4.v[i], xmin.v[i], xmax.v[i]);
}
...
template<unsigned int N, class T>
inline void
update_minmax(const Vec<N, T> & x, Vec<N, T> & xmin, Vec<N, T> & xmax)
{
...
template<unsigned int N, class T>
inline Vec<N, T>
mat_prod(const Vec<N * N, T> & x, const Vec<N, T> & y)
{
  Vec<N, T> ans;
  for(unsigned int i = 0; i < N; ++i)
    {
...
template<unsigned int N, class T>
inline Vec<N, T>
mat_prodT(const Vec<N * N, T> & x, const Vec<N, T> & y)
{
  Vec<N, T> ans;
  for(unsigned int i = 0; i < N; ++i)
    {
